...
let ClientesService = class ClientesService {
    ...
    async registrarCliente(dto, userId, id_empresa) {
    ...
    async consultarPorRut(rut, id_empresa) {
        if (!validarRut(rut)) {
    ...
    async editarFicha(id_cliente, dto, userId) {
    ...
    async marcarConflictivo(id_cliente, dto, userId) {
        const cliente = await this.prisma.cliente.findUnique({
            where: { id_cliente },
        });
        if (!cliente) {
    ...
    async listarClientes(id_empresa, page = 1, limit = 20) {
